...
public static partial class WKUtility {
    ...
    internal static SecureString Unprotect(string input) {
        ...
        if ((input.Length % 2) != 0) {
            throw new ArgumentException(nameof(input), "InvalidEncryptedString");
        }
    ...
    internal static EncryptionResult Encrypt(SecureString input, byte[] key, byte[]? iv) {
        ...
        if (key.Length != 16 && key.Length != 24 && key.Length != 32) {
            throw new ArgumentException(nameof(key), "InvalidKeyLength");
        }
        //
    ...
    internal static unsafe SecureString FromPlainTextString(string plainTextString) {
        ...
        if (plainTextString is null || plainTextString.Length == 0) {
